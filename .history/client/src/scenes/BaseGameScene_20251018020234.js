...
export class BaseGameScene extends Phaser.Scene {
  ...
  canMoveTo(sprite, nx, ny){
    ...
    // Slightly smaller hitbox for better navigation in tight spaces (was 0.28)
    const r = (sprite?.hbRadius != null) ? sprite.hbRadius : (this.hitboxRadius || (this.cell * 0.22));
    const pts = [
      {x:nx - r, y:ny - r},
      {x:nx + r, y:ny - r},
      {x:nx - r, y:ny + r},
  ...
  promptRunnerPowerSelection(onDone){
    ...
    powers.forEach((power, idx) => {
      ...
      const badge1 = this.add.text(x + (btnWidth / 2) - 24, y - 30, '1', {
        color:'#ffffff',
        fontSize:'14px',
        fontStyle:'bold',
      ...
      const badge2 = this.add.text(x + (btnWidth / 2) - 8, y - 30, '2', {
        color:'#ffffff',
        fontSize:'14px',
        fontStyle:'bold',
  ...
  firePlug(){
    ...
    const aim = (this.playerGunAim || this.playerAim) || { x: 1, y: 0 };
  ...
  update(_, delta){
    ...
    if (this.role !== 'runner') {
      this.considerAIRunnerPower(now);
    ...
    const moveHuman = (sprite, speed) => {
      // Only move if this is the user-controlled sprite
      const isUserControlled =
        (this.role === 'runner' && sprite === this.attacker) ||
        (this.role === 'plug' && sprite === this.defender);

      if (!isUserControlled) {
        console.error('moveHuman called on AI sprite!');
        return;
      }
      ...
      if (usingKeys){
        ...
        // Priority: horizontal > vertical (or use last pressed)
        ...
        // No keys: use straight-line movement direction
        const moveDir = this.playerMoveDir || { x: 1, y: 0 };
      ...
      // ONLY for the user-controlled sprite, never for AI
      if (!usingKeys){
        const isUserControlled =
          (this.role === 'runner' && sprite === this.attacker) ||
          (this.role === 'plug' && sprite === this.defender);
        ...
        if (isUserControlled) {
          ...
          // Apply corridor assist only to user-controlled sprite
          if (dir.x || dir.y) {
      ...
      if (this._cornerAssistActive && this._cornerAssistIntended) {
        ...
        if (!this.canMoveTo(sprite, testX, testY)) {
          ...
          for (const alt of perpDirs) {
            ...
            if (this.canMoveTo(sprite, altX, altY)) {
      ...
      if (this.canMoveTo(sprite, targetX, targetY)) {
        sprite.x = targetX;
        sprite.y = targetY;
        movedX = movedY = true;
      } else {
        // Try moving along each axis independently (wall sliding)
        if (this.canMoveTo(sprite, targetX, sprite.y)) {
          sprite.x = targetX;
          movedX = true;
        }
        if (this.canMoveTo(sprite, sprite.x, targetY)) {
          sprite.y = targetY;
          movedY = true;
        }
        ...
        if (!movedX && Math.abs(intendedDir.x) > 0 && Math.abs(dxTot) > 0.1) {
          ...
          if (this.canMoveTo(sprite, sprite.x + stepX, sprite.y)) {
            sprite.x += stepX;
            movedX = true;
          }
        }
        if (!movedY && Math.abs(intendedDir.y) > 0 && Math.abs(dyTot) > 0.1) {
          ...
          if (this.canMoveTo(sprite, sprite.x, sprite.y + stepY)) {
            sprite.y += stepY;
            movedY = true;
          }
        }
      ...
      // BUT never change the intended direction - just position better
      if ((!movedX || !movedY) && (Math.abs(vx) + Math.abs(vy) > 0)) {
        ...
        if (!movedX && Math.abs(intendedDir.x) > 0 && Math.abs(offsetY) > threshold) {
          const nudgeY = Math.sign(cy - sprite.y) * this.cell * 0.15 * dt;
          if (this.canMoveTo(sprite, sprite.x, sprite.y + nudgeY)) {
            sprite.y += nudgeY;
          }
        }
        ...
        if (!movedY && Math.abs(intendedDir.y) > 0 && Math.abs(offsetX) > threshold) {
          const nudgeX = Math.sign(cx - sprite.x) * this.cell * 0.15 * dt;
          if (this.canMoveTo(sprite, sprite.x + nudgeX, sprite.y)) {
            sprite.x += nudgeX;
          }
        }
      }
  ...
  updateDefenderAI(dt){
    ...
    if (updateStreetWarsPlugAI(this, dt)) {
    ...
    // Street Wars: Apply human-like shooting behaviors before aiming
    applyStreetWarsShootingBehavior(this);
  ...
  updateAIRunner(delta){
    ...
    updateStreetWarsRunnerAI(this, delta);
        ...
        if (who) {
          ...
          if (!this.canMoveTo(who, testX, testY)) {
            ...
            for (const alt of perpDirs) {
              ...
              if (this.canMoveTo(who, altX, altY)) {
                altDir = alt;
                break;
              }
            }

            // If we found a way around, mark this as corner-assist mode
            if (altDir) {
              this._cornerAssistActive = true;
              this._cornerAssistIntended = { x: nx, y: ny };
              this._cornerAssistTimeout = performance.now() + 800; // 0.8 seconds
            }
          }
        }

        // Update movement direction and track what player intended
        this.playerMoveDir = { x: nx, y: ny };
        this.playerIntendedDir = { x: nx, y: ny }; // Remember player's choice
        this.playerAim = { x: nx, y: ny };
        this.playerDrift = { x: nx, y: ny };
        this.playerGunAim = { x: nx, y: ny };
