...
export class BaseGameScene extends Phaser.Scene {
  ...
  create(){
    ...
    if (this.mode === 'pve') {
      ...
      if (this.role === 'plug') {
        this.aiRunnerTargetsBunkFirst = Math.random() < 0.45;
      } else {
  ...
  spawnWeaponBurst(origin, aim, weapon, group){
    ...
    pellets.forEach((offset) => {
      ...
      const rimThk = Math.max(1, Math.floor(this.cell * (isLightFloor ? 0.05 : 0.04)));
  ...
  updateBullets(delta){
    ...
    const step = (group)=>{
      group.getChildren().forEach(b=>{
        ...
        b.life -= delta;
        if (this.isBulletBlockedAtWorld(b.x, b.y) || b.life<=0){ impact(b.x, b.y, b._color || 0xffffff); b._glow?.destroy?.(); b._rim?.destroy?.(); b.destroy(); }
      });
    };
    step(this.bulletsA);
    step(this.bulletsD);
  ...
  applyRunnerAIDifficulty(){
    ...
    this.aiRunner.planEvery = Math.max(90, 240 - (round - 1) * 8);
    this.aiRunner.sprintMul = 1.05 + Math.min(0.4, (round - 1) * 0.02);
    this.aiRunner.sprintDist = 5 + Math.min(4, (round - 1) * 0.15);
    this.aiRunner.centerBias = 12 + Math.min(14, (round - 1) * 0.5);
    this.aiRunner.slideNudge = Math.min(0.4, 0.16 + (round - 1) * 0.012);
    this.aiRunner.keepDirMs = Math.max(160, 300 - (round - 1) * 6);
  }
