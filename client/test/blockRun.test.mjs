...
import {
  BLOCK_RUN_VERSION, POWER_IDS, beginBlockRun, createBlockRun, recordHouseClear, recordBlockDeath,
  recordMissionOutcome, blockRunStats
} from '../src/logic/blockRun.js';
let passed = 0;
function check(name, ok) { if (!ok) throw new Error(name); passed++; }

let run = beginBlockRun({}, 1, 1).state;
...
let powered = clear(clear(clear(beginBlockRun({}, 1, 1).state, 1, { powers: ['phase', 'dash'] }), 2, { powers: ['phase'] }), 3, { powers: ['decoy'] });
...
check('one press is not a habit', blockRunStats(clear(beginBlockRun({}, 1, 1).state, 1, { powers: ['dash'] }), 1).topPower === null);
check('a tie names nobody', blockRunStats(
  clear(clear(beginBlockRun({}, 1, 1).state, 1, { powers: ['dash', 'dash'] }), 2, { powers: ['phase', 'phase'] }), 1).topPower === null);
check('unknown powers are dropped', blockRunStats(clear(beginBlockRun({}, 1, 1).state, 1, { powers: ['laser', 'phase'] }), 1).powersUsed === 1);
check('powers are capped at the two a runner carries',
  blockRunStats(clear(beginBlockRun({}, 1, 1).state, 1, { powers: ['phase', 'dash', 'decoy'] }), 1).powersUsed === 2);

let bagged = clear(clear(beginBlockRun({}, 1, 1).state, 1, { bunk: true }), 2, { bunk: true });
check('bunk grabs are counted', blockRunStats(bagged, 1).bunks === 2 && blockRunStats(bagged, 1).cleanBags === false);
check('spawn swaps are counted', blockRunStats(clear(beginBlockRun({}, 1, 1).state, 1, { swapped: true }), 1).swaps === 1);
check('first-try houses are counted',
  blockRunStats(clear(clear(beginBlockRun({}, 1, 1).state, 1), 2, { deaths: 2 }), 1).firstTryHouses === 1);
...
let job = beginBlockRun({}, 1, 1).state;
