...
export class PvpScene extends Phaser.Scene {
  ...
  drawNeonArena(){
    ...
    for (let y=-1; y<=rows; y++){
      for (let x=-1; x<=cols; x++){
        ...
        // Arrange a 3x2 repeating plank pattern
        const idx = ((((x%3)+3)%3) + 3 * ((((y%2)+2)%2))) % WOOD.length;
        const key = WOOD[idx];
        const f = this.add.image(cx, cy, key).setDepth(1);
    ...
    const drawDefaultCell = (cx,cy)=>{
      ...
      const base = this.add.image(wx, wy, 'wall_fill').setDepth(3);
      base.setDisplaySize(cell, cell);
      this.walls.add(base);
      const n = isWall(cx, cy-1), e = isWall(cx+1,cy), s = isWall(cx,cy+1), w = isWall(cx-1,cy);
      const addEdge = (angle)=>{
        const edge = this.add.image(wx, wy, 'wall_edge').setDepth(4);
    ...
    for (let y=1; y<rows-1; y++){
      for (let x=1; x<cols-1; x++){
        ...
        if (!placed.couch && (straightH || straightV) && (w>=3 || h>=3)){
          ...
          if (straightH){
            ...
            for (let i=0;i<h;i++){
              ...
              this.add.image(wx, wy, key).setDepth(5).setAngle(90).setDisplaySize(cell, cell);
            }
          }
        ...
        if (!placed.table && ((w===2 && h===2) || (w===3 && h===2) || (w===2 && h===3))){
          ...
          this.add.image(wx0, wy0, 'f2_506').setDepth(5).setDisplaySize(cell, cell);
          ...
          for(const [dx,dy] of around){ const tx=cx0+dx, ty=cy0+dy; if (tx>=minx&&tx<=maxx&&ty>=miny&&ty<=maxy){
              ...
              this.add.image(pad.x + tx*cell + cell/2, pad.y + ty*cell + cell/2, k).setDepth(5).setDisplaySize(cell, cell);
          } }
        ...
        if (!placed.lazy && comp.length===1){
          ...
          this.add.image(wx, wy, k).setDepth(5).setDisplaySize(cell, cell);
  ...
  placeGetawayCar(){
    ...
    car.setDisplaySize(carLen, this.cell*1.4);
    car.setAngle(ang);
    this.car = car;
  ...
  makeObjectives(stashCell, extractCell){
    ...
    this.extractHalo = this.add.graphics().setDepth(999).setVisible(true);
