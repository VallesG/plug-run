...
function updateAvatarVisuals(scene, dt){
  const step = (who, prevKey)=>{
    ...
    if (who.kind === 'plug'){
      const aim = (scene.role === 'plug') ? scene.playerAim : scene.aiAim;
      if (aim){
...
export class PvpScene extends Phaser.Scene {
  ...
  create(){
    ...
    // split aims (player vs AI)
    this.playerAim = { x:1, y:0 };
    this.aiAim     = { x:1, y:0 };

    // desktop keyboard quick-aim (player)
    this.input.keyboard.on('keydown-W', ()=> this.playerAim = {x:0, y:-1});
    this.input.keyboard.on('keydown-S', ()=> this.playerAim = {x:0, y: 1});
    this.input.keyboard.on('keydown-A', ()=> this.playerAim = {x:-1,y: 0});
    this.input.keyboard.on('keydown-D', ()=> this.playerAim = {x: 1,y: 0});
    ...
    // Enforce mobile mode (override detection)
    this.forceMobile = true;
    if (this.forceMobile) this.isDesktop = false;
    ...
    // Runner power stats (Phase & Decoy)
    this.runnerPowerStats = {
      phase: { label: 'Phase Dash', description: 'Blink forward and become intangible briefly.', cooldown: 7000, duration: 420, dashDist: this.cell * 3.5, fadeAlpha: 0.35 },
      decoy: { label: 'Decoy', description: 'Deploy a hologram runner that pulls aggro.', cooldown: 9000, life: 5500, speed: this.runnerSpeed * 0.9 }
    };
  ...
  startMatch(role){
    ...
    // Initialize human auto-drift: start in a random straight direction
  ...
  promptRunnerPowerSelection(onDone){
    ...
    const options = ['phase','decoy']; // fixed: decoy not "dash"
    this.roundPausedForMenu = true;
    const modal = this.showModal({
  ...
  performRunnerPower(power){
    ...
    if (power === 'phase'){
      ...
      const dash = stats.dashDist || this.cell * 3;
      const targetX = Phaser.Math.Clamp(this.attacker.x + dir.x * dash, this.pad.x + this.cell*0.5, this.pad.x + this.cols*this.cell - this.cell*0.5);
      const targetY = Phaser.Math.Clamp(this.attacker.y + dir.y * dash, this.pad.y + this.cell*0.5, this.pad.y + this.rows*this.cell - this.cell*0.5);
      this.attacker.x = targetX;
      this.attacker.y = targetY;
      this.attacker.setAlpha(stats.fadeAlpha ?? 0.45);
      this.phaseActiveUntil = now + (stats.duration || 420);
    } else if (power === 'decoy'){
      this.destroyDecoySprite();
      const decoy = makeRunnerSprite(this, this.attacker.x, this.attacker.y, this.cell)
        .setDepth(this.attacker.depth - 0.5)
        .setAlpha(0.65);
      // bluish tint to look hologram-y
      decoy.list?.forEach?.(child => child.setTint?.(0x93c5fd));
  ...
  update(_, delta){
    ...
    const moveHuman = (sprite, speed) => {
      ...
      if (usingKeys){
        ...
        // No keys: fall back to player's own aim or persistent drift (never AI aim)
        const aim = this.playerAim || this.playerDrift || this._initDrift || { x: 1, y: 0 };
