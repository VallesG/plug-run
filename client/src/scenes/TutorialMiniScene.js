...
export class TutorialMiniScene extends Phaser.Scene {
  ...
  init(data){
    // Fixed grid matching BaseGameScene — same layout on every device
    this.cell = 24; // provisional; recomputed from viewport
    this.cols = 16;
    this.rows = 35;
    this.pad = { x: 0, y: 0 };
  ...
  ensureResizeListener(){
    ...
    // RESIZE mode: rebuild the current stage when the viewport genuinely
    // changes (rotation / window drag), ignoring small jitter.
    ...
    this._onResizeCb = (gameSize) => {
      ...
      this._lastW = gameSize.width;
      this._lastH = gameSize.height;
      clearTimeout(this._resizeTimer);
      this._resizeTimer = setTimeout(() => this.startStage(this.stageIdx || 1), 250);
    };
  ...
  computeLayout(){
    ...
    // Center the fixed grid; margins handled by scene background
    this.pad = {
      x: Math.max(0, Math.floor((usableW - this.worldWidth)  / 2)),
      y: Math.max(0, Math.floor((usableH - this.worldHeight) / 2))
    };
