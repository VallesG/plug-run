...
export class BaseGameScene extends Phaser.Scene {
  ...
  init(data) {
    // FIXED GRID — the fairness invariant. Every player gets the same
    // 16x35 maze regardless of device (matches the old 390x844 @ 24px
    // layout, so existing daily seeds keep producing the same mazes).
    // Cell pixel size adapts to the screen in computeLayoutFromViewport.
    this.cols = BaseGameScene.GRID_COLS;
    this.rows = BaseGameScene.GRID_ROWS;
    this.cell = 24; // provisional; recomputed from viewport before drawing
    this.pad  = { x: 0, y: 0 };
    this.timerMs = 90_000;
  ...
  create(){
    ...
    this._onResizeCb = (gameSize) => {
      const dw = Math.abs(gameSize.width  - this._lastLayoutW);
  ...
  async refreshSidebarStats() {
    ...
    try {
      ...
      console.warn('[BaseGameScene] Failed to refresh sidebar stats:', err);
      updateStats({
        totalRounds: 0,
        totalStash: 0,
        repEarned: 0,
  ...
  applyMusicRamp(){
    ...
    if (musicKey === 'bg_plug') {
      ...
      baseVol = 0.22;
      maxVol = 0.48;
    } else if (musicKey === 'bg_beat5' || musicKey === 'bg_beat6' || musicKey === 'bg_beat7') {
      baseVol = 0.22;
      maxVol = 0.48;
    }
