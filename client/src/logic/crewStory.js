...
export const CREW_STORIES = Object.freeze({
  crossline: {
    ...
    chapters: [
      ...
      ['Every Corner Counts', 'We used to have a wall full of guesses. Now we have routes, names, and people waiting on us.', 'Finish this block too. Fifteen real bags keeps the street network from becoming another empty promise.', 'Another complete block. You keep turning those marks on the wall into something the crew can use.', 'No shortcut replaces finishing. That is the part of your route I keep showing new runners.'],
      ['The Street Answers', 'Remember that radio nobody answered? I do. Crossline is going to keep earning every reply.', 'All fifteen real bags. The network is never finished; tonight we keep it alive.', 'Fifteen out. You helped turn dead air into a street that answers back.', 'There is room on the board for the next block. Our story keeps moving with you.']
    ],
    jobLines: [
      'That tube is for the first route board. I want names and turns we can trust, not another wall full of guesses.',
      'The Mercer route needs a proper record. Bring the tube if you can; the fifteen real bags still finish the block.',
  ...
  'iron-row': {
    ...
    jobLines: [
      ...
      'The night shift needs access too. Get the keys if you can; the real bag remains the job that finishes the house.',
  ...
  afterlight: {
    ...
    chapters: [
      ['After Closing', 'The street goes dark and everybody acts like the night belongs to somebody else. I am tired of that.', 'Bring out fifteen real bags. A whole block for the crew, and we start planning a night of our own.', 'Fifteen bags. Now this night has a beginning we actually earned.', 'I have the first setup list. Next we need more than a name on a wall.'],
      ...
      ['Everybody Stays', 'The first arrivals are easy to imagine. I am thinking about the people who stay until the last light.', 'All fifteen real bags. Help the crew make something worth sticking around for.', 'Fifteen out. You keep giving this crew a reason to take the next night seriously.', 'There is one more setup to plan. I want the last piece to feel as good as the first.'],
      ['Our Kind of Night', 'This started with a dark street and a stubborn idea. Now I can see what we were trying to make.', 'One whole block, fifteen real bags. We earn the next night the same way we earned the first.', 'All fifteen. You helped make Afterlight more than a name people saw after closing.', 'Tomorrow has its own list. Tonight, take the compliment. You finished.']
    ],
    jobLines: [
      'That marker is the first little piece of the setup. Vee can sketch the night; somebody has to carry it out.',
      'The wall needs a name people can spot from the corner. Bring the marker if you can, and do not forget the real bag.',
...
export function crewChapter(gangID, chapter = 0) {
  ...
  return {
    ...
    context: story.ongoing, goal: 'Bring out all fifteen real bags. A finished block is what moves our story forward.',
    primaryFinish: 'Fifteen real bags, another whole block. Chapter ' + (n + 1) + ' is finished. Take a breath; the crew has your next run ready.',
...
export function crewConsultationPages(gangID, chapter, beat, measuredText) {
  ...
  if (beat === 'tease') return [measuredText, 'We are still working on ' + story.title + '. The extra object helps the job; all fifteen real bags finish the chapter.'];
  if (beat === 'checkin-2') return [measuredText, 'Three houses left in this block. Bring out the last three real bags and we can talk about what comes next.'];
  return [measuredText];
}
