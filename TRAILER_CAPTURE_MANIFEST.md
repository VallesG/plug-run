...
## Crew trailers and Block Rivals vs Jev (`plug-run-{crossline,iron-row,afterlight,rivals}.mp4`)

Same style as vertical v4: hard cuts, game SFX, `gameplay_beat6`, poster end card. Plans: `plan-crossline.json`, `plan-iron-row.json`, `plan-afterlight.json` (from `make-crew-plans.cjs`) and `plan-rivals.json` (from `make-rivals-plan.cjs`).

- **Crew story beats** were filmed with `beat-take.mjs`: a scratch profile that runs with the crew and is on a given Season 1 chapter (only local save state: crew choice, chapter counter, tutorial done), started at the door the beat plays before. The contact panel, the house after it and, for door 15, the two-Plug warning all play from the game's own flow; gates are real taps. Beats used: Crossline ch1 door 9, ch3 door 10, ch10 doors 9 and 15; Iron Row ch1 door 9, ch4 door 9, ch9 door 13, ch10 doors 9 and 15; Afterlight ch1 door 4, ch6 door 9, ch10 doors 9 and 15. Crew picks and chapter-1 openers come from the v3 story takes.
- **Block Rivals vs Jev:** our side from the calm-bot Rivals takes (entrance, lobby, countdown, race, the "JEV FINISHED" alert); Jev's side from the Jev recording sessions' own videos (`recordings/jev/rival-v1` and `all-maps-v5`, git-ignored), cut at clean power activations with SFX from their event logs. The result card ("JEV WINS", 6/7 vs 1:01.5) is a real loss filmed by the `result` take, with SHARE MY RUN unticked in the lobby (a scratch profile has no player identity to share with).
- Every gameplay clip from our takes passes the twitch screen (at most 2 reversals per half second).
